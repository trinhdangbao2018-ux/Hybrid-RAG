...
class KeywordRetriever:
    ...
    def __init__(self, chunks: list[Chunk]):
        ...
        self.bm25.index(corpus_tokens)              # this task analyse every words, there position, and how many time it appear
    ...
    def search(self, query: str, top_n: int) -> list[tuple[str, float]]:
        ...
        return [(self.chunks[i].chunk_id, float(s)) for i, s in zip(ids[0], scores[0]) if s > 0]     # List of (chunk_id: BM25 point)
                                                                                                             # if scores[0] > 0: Skip 0 point chunk, basically no value